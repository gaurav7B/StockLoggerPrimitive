...
{
    public class _11Service : BackgroundService
    {
        private readonly ILogger<_11Service> _logger;
        private readonly HttpClient _httpClient;
        private bool _isRunning = true;

        public _11Service(ILogger<_11Service> logger)
        {
            _logger = logger;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                var stocks = (await GetStockTickerExchanges())?.Select(x => new { x.Id, x.Ticker, x.Exchange }).Where(x => x.Id == 11).ToArray();
        ...
        private async Task CandelMaker(int Id, string ticker, string exchange)
        {
            try
            {
                ...
                while (_isRunning)
                {
                    ...
                    {
                        // Make API request
                        var response = await _httpClient.GetAsync($"https://localhost:44364/Stock/GetStockPriceByTickerFor_11Service?ticker={ticker}&exchange={exchange}");
