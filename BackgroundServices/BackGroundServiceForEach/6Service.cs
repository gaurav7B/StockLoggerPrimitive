...
namespace StockLogger.BackgroundServices.BackGroundServiceForEach
{
    ...
    {
        private readonly ILogger<_6Service> _logger;
        private readonly HttpClient _httpClient;
        private bool _isRunning = true;

        public _6Service(ILogger<_6Service> logger)
        {
            _logger = logger;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                var stocks = (await GetStockTickerExchanges())?.Select(x => new { x.Id, x.Ticker, x.Exchange }).Where(x => x.Id == 6).ToArray();
        ...
        private async Task CandelMaker(int Id, string ticker, string exchange)
        {
            try
            {
                ...
                while (_isRunning)
                {
                    ...
                    {
                        // Make API request
                        var response = await _httpClient.GetAsync($"https://localhost:44364/Stock/GetStockPriceByTickerFor_6Service?ticker={ticker}&exchange={exchange}");
