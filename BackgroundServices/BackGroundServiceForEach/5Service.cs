...
namespace StockLogger.BackgroundServices.BackGroundServiceForEach
{
    ...
    {
        private readonly ILogger<_5Service> _logger;
        private readonly HttpClient _httpClient;
        private bool _isRunning = true;

        public _5Service(ILogger<_5Service> logger)
        {
            _logger = logger;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                var stocks = (await GetStockTickerExchanges())?.Select(x => new { x.Id, x.Ticker, x.Exchange }).Where(x => x.Id == 5).ToArray();
        ...
        private async Task CandelMaker(int Id, string ticker, string exchange)
        {
            try
            {
                ...
                while (_isRunning)
                {
                    ...
                    {
                        // Make API request
                        var response = await _httpClient.GetAsync($"https://localhost:44364/Stock/GetStockPriceByTickerFor_5Service?ticker={ticker}&exchange={exchange}");
