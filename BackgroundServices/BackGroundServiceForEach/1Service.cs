...
{
    public class _1Service : BackgroundService
    {
        private readonly ILogger<_1Service> _logger;
        private readonly HttpClient _httpClient;
        private bool _isRunning = true;

        public _1Service(ILogger<_1Service> logger)
        {
            _logger = logger;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                var stocks = (await GetStockTickerExchanges())?.Select(x => new { x.Id, x.Ticker, x.Exchange }).Where(x => x.Id >= 1 && x.Id <= 2).ToArray();
        ...
        public async Task<StockPricePerSec> GetLastStockPrice(List<StockPricePerSec> stockList, string tickerForAPI, DateTime lastDateTime)
        {
            ...
            var url = $"https://localhost:44364/api/StockPricePerSec/by-datetime1?datetime={formattedDateTime}&ticker={encodedTicker}";
        ...
        private async Task CandelMaker(int Id, string ticker, string exchange)
        {
            try
            {
                ...
                while (_isRunning)
                {
                    ...
                    {
                        // Make API request
                        var response = await _httpClient.GetAsync($"https://localhost:44364/Stock/GetStockPriceByTickerFor_1Service?ticker={ticker}&exchange={exchange}");
