...
namespace StockLogger.Controllers.API_Controllers
{
    ...
    [ApiController]
    public class Candel10minController : ControllerBase
    {
        private readonly StockLoggerDbContext _context;

        public Candel10minController(StockLoggerDbContext context)
        {
            _context = context;
        }

        [HttpPost]
        public async Task<ActionResult> CreateCandel(Candel10min candel)
        {
            var existingCandel = await _context.Candel10min
            ...
            _context.Candel10min.Add(candel);
            await _context.SaveChangesAsync();
            return Ok();
        }
        ...
        // READ: api/candel
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Candel10min>>> GetCandels()
        {
            return await _context.Candel10min.ToListAsync();
        }

        [HttpGet("recentThree")]
        public async Task<ActionResult<IEnumerable<Candel10min>>> GetRecentCandels(string ticker, string exchange)
        {
            // Validate input
            ...
            var recentCandels = await _context.Candel10min
        ...
        //For CUP AND HANDEL 
        // READ: api/Candel10min/recentTen?ticker={ticker}&exchange={exchange}
        [HttpGet("recentTen")]
        public async Task<ActionResult<IEnumerable<Candel10min>>> GetRecentTenCandels(string ticker, string exchange)
        {
            // Validate input
            if (string.IsNullOrEmpty(ticker) || string.IsNullOrEmpty(exchange))
            {
        ...
        // READ (Single Item): api/Candel10min/{id}
        [HttpGet("{id}")]
        public async Task<ActionResult<Candel10min>> GetCandel(long id)
        {
            var candel = await _context.Candel10min.FindAsync(id);
        ...
        // UPDATE: api/Candel10min/{id}
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateCandel(long id, Candel10min candel)
        {
            if (id != candel.Id)
            {
                return BadRequest();
        ...
        public async Task<IActionResult> DeleteCandel()
        {
            ...
            await _context.Database.ExecuteSqlRawAsync("TRUNCATE TABLE Candel10min");
        ...
        {
            return _context.Candel10min.Any(e => e.Id == id);
        }
